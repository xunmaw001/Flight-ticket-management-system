...
<body>
		...
		<div id="contentText">
				...
				<div class="container mt-0">
					<div class="row breadcrumb-bar">
						<div class="col-md-6">
							<h3 class="block-title">编辑航班机票</h3>
						</div>
						<div class="col-md-6">
							<ol class="breadcrumb">
								...
								<li class="breadcrumb-item active"><span>编辑航班机票</span></li>
							</ol>
						</div>
					</div>
				</div>
				...
				<div class="row">
					...
					<div class="col-md-12">
						<div class="widget-area-2 lochana-box-shadow">
							<h3 class="widget-title">航班机票信息</h3>
							<form id="addOrUpdateForm">
								<div class="form-row">
									<input id="updateId" name="id" type="hidden">
									<div class="form-group col-md-6">
										<label>航班编号</label>
										<input id="hangbanbianhao" name="hangbanbianhao" class="form-control" value="" readonly>
									</div>
									<div class="form-group col-md-6">
										<label>航空公司</label>
									...
									<div class="form-group col-md-6">
										<label>乘机地点</label>
										<input name="chengjididian" id="chengjididian" name="chengjididian" class="form-control" placeholder="乘机地点">
									</div>
									<div class="form-group col-md-6">
										<label>封面图片</label>
									...
									<div class="form-group col-md-6">
										<label>机票票价</label>
										<input name="jipiaopiaojia" id="jipiaopiaojia" name="jipiaopiaojia" class="form-control" placeholder="机票票价">
									</div>
									<div class="form-group col-md-6">
										<label>出发地</label>
										<input name="chufadi" id="chufadi" name="chufadi" class="form-control" placeholder="出发地">
									</div>
									<div class="form-group col-md-6">
										<label>目的地</label>
										<input name="mudedi" id="mudedi" name="mudedi" class="form-control" placeholder="目的地">
									</div>
									<div class="form-group col-md-6">
										<label>飞行时长</label>
										<input name="feixingshizhang" id="feixingshizhang" name="feixingshizhang" class="form-control" placeholder="飞行时长">
									</div>
									<div class="form-group col-md-6">
										<label>类型</label>
									...
									<div class="form-group col-md-6">
										<label>出发时间</label>
										<div id="chufashijianDate" class="input-append date form_datetime">
											<input id="chufashijian-input" name="chufashijian" size="20" type="text" readonly>
	...
	<script>
		...
		function hangbanbianhaoUuid(){
			if($("#hangbanbianhao").val() == null || $("#hangbanbianhao").val() == "null" || $("#hangbanbianhao").val() == ""){
				$("#hangbanbianhao").attr("value",Math.round(new Date().getTime()/1000));
			}
		}				
		  ...
		  beforeCreate: function(){
			  var id = window.sessionStorage.getItem("id");
				if(id != null && id != "" && id != "null"){
					$.ajax({ 
		                type: "GET",
		                url: baseUrl + "hangbanjipiao/info/" + id,
				...
				if(crossrefid && crossuserid) {
					...
					http("hangbanjipiao/page","GET",{
						...
						if(res.data.total >= crossoptnum){
							...
							httpJson("hangbanjipiao/"+urlParam,"POST",data,(res)=>{
								if(res.code == 0){
		...
		function getDetails() {
			...
			if(id != null && id != "" && id != "null"){
				updateId = id;
				window.sessionStorage.removeItem('id');
				http("hangbanjipiao/info/" + id,"GET",{},(res)=>{
					if(res.code == 0){
						ruleForm = res.data
						// 是/否下拉框回显
						setSelectOption();
		...
		function dateTimePick(){
			...
			$("#chufashijian-input").val(getCurDateTime());
			$("#chufashijianDate").datetimepicker({ 
				autoclose: true,
		...
		function dataBind(){
			...
			$("#hangbanbianhao").val(ruleForm.hangbanbianhao);	
			$("#hangkonggongsi").val(ruleForm.hangkonggongsi);	
			$("#hangban").val(ruleForm.hangban);	
			$("#chengjididian").val(ruleForm.chengjididian);	
			$("#fengmiantupian-input").val(ruleForm.fengmiantupian);
			$("#jipiaopiaojia").val(ruleForm.jipiaopiaojia);	
			$("#chufadi").val(ruleForm.chufadi);	
			$("#mudedi").val(ruleForm.mudedi);	
			$("#feixingshizhang").val(ruleForm.feixingshizhang);	
			...
			$("#chufashijian-input").val(ruleForm.chufashijian);
		...
		if(crossTableName != null && crossTableName != '' && crossTableId != null && crossTableId != '' && crossTableId != -1){
			http(crossTableName + "/info/" + crossTableId,"GET",{},(res)=>{
				if(res.code == 0){
					...
					if(res.data != null && res.data != '' && res.data.hangbanbianhao != null && res.data.hangbanbianhao != ''){

						$("#hangbanbianhao").val(res.data.hangbanbianhao);
						$("#hangbanbianhao").attr('readonly','readonly');
					}
					...
					if(res.data != null && res.data != '' && res.data.chengjididian != null && res.data.chengjididian != ''){

						$("#chengjididian").val(res.data.chengjididian);
						$("#chengjididian").attr('readonly','readonly');
					}
					...
					if(res.data != null && res.data != '' && res.data.jipiaopiaojia != null && res.data.jipiaopiaojia != ''){

						$("#jipiaopiaojia").val(res.data.jipiaopiaojia);
						$("#jipiaopiaojia").attr('readonly','readonly');
					}
					if(res.data != null && res.data != '' && res.data.chufadi != null && res.data.chufadi != ''){

						$("#chufadi").val(res.data.chufadi);
						$("#chufadi").attr('readonly','readonly');
					}
					if(res.data != null && res.data != '' && res.data.mudedi != null && res.data.mudedi != ''){

						$("#mudedi").val(res.data.mudedi);
						$("#mudedi").attr('readonly','readonly');
					}
					if(res.data != null && res.data != '' && res.data.feixingshizhang != null && res.data.feixingshizhang != ''){

						$("#feixingshizhang").val(res.data.feixingshizhang);
						$("#feixingshizhang").attr('readonly','readonly');
					}
					...
					if(res.data != null && res.data != '' && res.data.chufashijian != null && res.data.chufashijian != ''){

						$("#chufashijian-input").val(res.data.chufashijian);
						$('#chufashijianFileName').val(res.data.chufashijian)
					}
