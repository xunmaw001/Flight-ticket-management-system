...
<body>
		...
		<div id="content">
				...
				<div class="container mt-0">
					<div class="row breadcrumb-bar">
						<div class="col-md-6">
							<h3 class="block-title">机票预订管理</h3>
						</div>
						<div class="col-md-6">
							<ol class="breadcrumb">
								...
								<li class="breadcrumb-item"><span>机票预订管理</span></li>
								<li class="breadcrumb-item active"><span>机票预订列表</span></li>
							</ol>
						</div>
					</div>
				</div>
				...
				<div class="row">
					...
					<div class="col-md-12">
						<div class="widget-area-2 lochana-box-shadow">
							<h3 class="widget-title">机票预订列表</h3>							
							<div class="table-responsive mb-3" id="tableDiv">
								<div class="col-sm-12">
									<label>
										航空公司
									</label>
									<input type="text" id="hangkonggongsiSearch" class="form-control form-control-sm" placeholder="请输入航空公司" aria-controls="tableId">
									<label>
										出发地
									</label>
									<input type="text" id="chufadiSearch" class="form-control form-control-sm" placeholder="请输入出发地" aria-controls="tableId">
									<label>
										目的地
									</label>
									<input type="text" id="mudediSearch" class="form-control form-control-sm" placeholder="请输入目的地" aria-controls="tableId">
									<label>
										用户账号
									</label>
	...
	<script>
		...
		function search(){
			searchForm = { key: ""};
			if($('#hangkonggongsiSearch').val() != null && $('#hangkonggongsiSearch').val() != ''){
				searchForm.hangkonggongsi ="%" +  $('#hangkonggongsiSearch').val() + "%";
			}
			
			if($('#chufadiSearch').val() != null && $('#chufadiSearch').val() != ''){
				searchForm.chufadi ="%" +  $('#chufadiSearch').val() + "%";
			}
			
			if($('#mudediSearch').val() != null && $('#mudediSearch').val() != ''){
				searchForm.mudedi ="%" +  $('#mudediSearch').val() + "%";
			}
        ...
        function getDataList() {
			http("jipiaoyuding/page","GET",{
				page: pageIndex,
				limit: pageSize,
				sort: sortColumn,
				order: sortOrder,
				hangkonggongsi : searchForm.hangkonggongsi,	
				chufadi : searchForm.chufadi,	
				mudedi : searchForm.mudedi,	
        ...
        function remove(id) { 
            ...
            if (mymessage == true) {
				...
				httpJson("jipiaoyuding/delete","POST",paramArray,(res)=>{
					if(res.code == 0){
						getDataList();		
						alert('删除成功');
					}
	...
	function crossTable(obj,crossTableName,crossOptAudit,statusColumnName,tips,statusColumnValue){
		...
		window.sessionStorage.setItem('crossTableName',"jipiaoyuding");
        ...
        //跳转到支付页面
        function pay(item){
            http("jipiaoyuding/alipay","GET",{
          tradeno: item.dingdanbianhao,
          subject: item.hangkonggongsi,
          totalamount: item.zongjia,
            },(res)=>{
                if(res.code == 0){
